...
ALTER TABLE sc_class_course
  ...
  ADD COLUMN IF NOT EXISTS course_level_requirement varchar(100) DEFAULT NULL COMMENT '课程等级要求' AFTER selection_group_limit,
...
SET business_type = COALESCE(NULLIF(business_type, ''), '专科'),
    ...
    selection_option_name = COALESCE(selection_option_name, ''),
    selection_group_limit = CASE WHEN selection_group_limit IS NULL OR selection_group_limit <= 0 THEN 1 ELSE selection_group_limit END,
