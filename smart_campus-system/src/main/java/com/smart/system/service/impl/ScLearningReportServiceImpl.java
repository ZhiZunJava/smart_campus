...
public class ScLearningReportServiceImpl implements IScLearningReportService {
    ...
    public ScLearningReport selectScLearningReportByReportId(Long reportId) {
        return scLearningReportMapper.selectScLearningReportByReportId(reportId);
    }

    public List<ScLearningReport> selectScLearningReportList(ScLearningReport scLearningReport) {
        return scLearningReportMapper.selectScLearningReportList(scLearningReport);
    }

    public int insertScLearningReport(ScLearningReport scLearningReport) {
        return scLearningReportMapper.insertScLearningReport(scLearningReport);
    }

    public int updateScLearningReport(ScLearningReport scLearningReport) {
        return scLearningReportMapper.updateScLearningReport(scLearningReport);
    }

    public int deleteScLearningReportByReportIds(Long[] reportIds) {
        return scLearningReportMapper.deleteScLearningReportByReportIds(reportIds);
    }

    public int deleteScLearningReportByReportId(Long reportId) {
        return scLearningReportMapper.deleteScLearningReportByReportId(reportId);
    }
    ...
    public ScLearningReport generateReport(Long userId, String reportType) {
        ...
        BigDecimal avgProgress = behaviorCount == 0 ? BigDecimal.ZERO
                : totalProgress.divide(BigDecimal.valueOf(behaviorCount), 2, RoundingMode.HALF_UP);
    ...
    private String buildReportJson(int behaviorCount, int totalDuration, BigDecimal avgProgress,
            ScLearningProfile profile) {
        return String.format(
                "{\"behaviorCount\":%d,\"totalDurationSeconds\":%d,\"avgProgress\":%s,\"hasProfile\":%s,\"abilityLevel\":\"%s\",\"activeScore\":%s,\"masteryScore\":%s,\"riskScore\":%s}",
                behaviorCount,
                totalDuration,
                ...
                profile == null || profile.getActiveScore() == null ? "null"
                        : profile.getActiveScore().setScale(2, RoundingMode.HALF_UP).toPlainString(),
                profile == null || profile.getMasteryScore() == null ? "null"
                        : profile.getMasteryScore().setScale(2, RoundingMode.HALF_UP).toPlainString(),
                profile == null || profile.getRiskScore() == null ? "null"
                        : profile.getRiskScore().setScale(2, RoundingMode.HALF_UP).toPlainString());
    }
}
