...
public class ScLearningWarningServiceImpl implements IScLearningWarningService {
    ...
    public ScLearningWarning selectScLearningWarningByWarningId(Long warningId) {
        return scLearningWarningMapper.selectScLearningWarningByWarningId(warningId);
    }

    public List<ScLearningWarning> selectScLearningWarningList(ScLearningWarning scLearningWarning) {
        return scLearningWarningMapper.selectScLearningWarningList(scLearningWarning);
    }

    public int insertScLearningWarning(ScLearningWarning scLearningWarning) {
        return scLearningWarningMapper.insertScLearningWarning(scLearningWarning);
    }

    public int updateScLearningWarning(ScLearningWarning scLearningWarning) {
        return scLearningWarningMapper.updateScLearningWarning(scLearningWarning);
    }

    public int deleteScLearningWarningByWarningIds(Long[] warningIds) {
        return scLearningWarningMapper.deleteScLearningWarningByWarningIds(warningIds);
    }

    public int deleteScLearningWarningByWarningId(Long warningId) {
        return scLearningWarningMapper.deleteScLearningWarningByWarningId(warningId);
    }
    ...
    public ScLearningWarning buildWarning(Long userId, Long courseId) {
        ...
        if (!existingWarnings.isEmpty()) {
            ...
            existingWarning
                    .setWarningLevel(profile.getRiskScore().compareTo(BigDecimal.valueOf(70)) >= 0 ? "HIGH" : "MEDIUM");
    ...
    public int processWarning(Long warningId, String processStatus) {
        ...
        warning.setProcessStatus(
                processStatus == null || processStatus.trim().isEmpty() ? "PROCESSED" : processStatus.trim());
