...
public class ScStudyRecordServiceImpl implements IScStudyRecordService
{
    ...
    @Override
    public ScStudyRecord selectScStudyRecordByRecordId(Long recordId) { return scStudyRecordMapper.selectScStudyRecordByRecordId(recordId); }

    @Override
    public List<ScStudyRecord> selectScStudyRecordList(ScStudyRecord scStudyRecord) { return scStudyRecordMapper.selectScStudyRecordList(scStudyRecord); }

    @Override
    public int insertScStudyRecord(ScStudyRecord scStudyRecord) { return scStudyRecordMapper.insertScStudyRecord(scStudyRecord); }

    @Override
    public int updateScStudyRecord(ScStudyRecord scStudyRecord) { return scStudyRecordMapper.updateScStudyRecord(scStudyRecord); }

    @Override
    public int deleteScStudyRecordByRecordIds(Long[] recordIds) { return scStudyRecordMapper.deleteScStudyRecordByRecordIds(recordIds); }

    @Override
    public int deleteScStudyRecordByRecordId(Long recordId) { return scStudyRecordMapper.deleteScStudyRecordByRecordId(recordId); }
    ...
    public int recordBehavior(ScStudyRecord scStudyRecord)
    {
        ...
        if (rows > 0 && scStudyRecord.getResourceId() != null && StringUtils.isNotEmpty(scStudyRecord.getBehaviorType()))
        {
