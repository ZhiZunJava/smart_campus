<template>
  <StudentAffairPageFrame
    ...
    :title="presentation?.title || '学科竞赛服务'"
    :subtitle="presentation?.subtitle || '学科竞赛报名、指导教师和获奖记录'"
    :metrics="metrics"
  ...
  >
    <section v-if="windowCards.length" class="service-section">
      <div class="section-head">
        <h3>申报时段</h3>
        <p>学科竞赛按学期开放申报，申报范围和时段由老师或管理员配置。</p>
      </div>
    ...
    <section class="service-section">
      <div class="section-head">
        <h3>我的竞赛记录</h3>
        <p>查看竞赛名称、竞赛级别、指导教师和获奖情况。</p>
      </div>
      <div v-if="competitionRecords.length" class="record-list">
        <div v-for="item in competitionRecords" :key="item.requestId" class="record-row">
          <div class="record-row__main">
            <strong>{{ item.competitionName }}</strong>
            <span class="record-row__sub">{{ item.competitionLevelLabel }}</span>
          </div>
          <div class="record-row__detail">
            <span v-if="item.mentorTeacherId">指导教师：{{ item.mentorTeacherId }}</span>
            <span v-if="item.award"><span class="sep">·</span>获奖：{{ item.award }}</span>
          </div>
...
const { loading, detail, presentation, templates, requests, stats, guideSteps } = useStudentAffairDetail(computed(() => props.categoryCode || 'SUBJECT_COMPETITION'))
...
const competitionRecords = computed(() => requests.value.filter((item: any) => item.categoryCode === 'SUBJECT_COMPETITION').map((item: any) => ({
  requestId: item.requestId,
  competitionName: item.parsedFormData.competitionName || '竞赛报名',
  competitionLevelLabel: resolveOptionLabel(competitionLevelOptions, item.parsedFormData.competitionLevel, '竞赛报名'),
  mentorTeacherId: item.parsedFormData.mentorTeacherId,
  award: item.parsedFormData.award,
  requestStatus: item.requestStatus,
  submittedTime: item.submittedTime,
})))

const metrics = computed(() => ([
  { label: '累计参赛', value: competitionRecords.value.length, hint: '竞赛报名记录', tone: '' },
  { label: '已通过', value: competitionRecords.value.filter((item: any) => item.requestStatus === 'APPROVED').length, hint: '已通过审核', tone: 'success' },
  { label: '待审核', value: Number(stats.value.pending || 0), hint: '等待教师或辅导员审核', tone: 'warning' },
  { label: '获奖次数', value: competitionRecords.value.filter((item: any) => item.award).length, hint: '已获奖记录', tone: 'info' },
]))
...
const summaryCards = computed(() => ([
  {
    ...
    lines: [templates.value.length ? '你在学科竞赛报名范围内' : '不在申请范围，请联系辅导员'],
  },
]))
...
const actions = computed(() => {
  return templates.value
    ...
    .map((t: any) => ({ label: '发起竞赛报名', template: t, type: 'primary' as const }))
})
