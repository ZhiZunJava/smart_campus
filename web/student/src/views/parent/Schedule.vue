<template>
  <div class="portal-page schedule-page">
    <div class="portal-section-title">
      <h3>孩子课表</h3>
      <el-tag type="primary">家长教务视图</el-tag>
    </div>
    ...
    <el-card class="portal-card schedule-shell">
      <div class="schedule-filters">
        ...
        <div class="schedule-meta">
          学期起始日期: {{ formatDate(currentTermMeta.startDate) || '-' }}
        </div>
      </div>
              ...
              <template v-for="day in weekDays" :key="`${row.key}-${day.value}`">
                ...
                >
                  ...
                  >
                    ...
                    <div class="lesson-name">{{ getActivity(row.key, day.value).teacherName || formatTeachers(getActivity(row.key, day.value)) }}</div>
...
import { getParentDashboard, getPortalParentSchedule, listPortalTermOptions } from '@/api/portal'
...
.schedule-shell {
  ...
  border-radius: 0;
  background:
    radial-gradient(circle at top left, rgba(121, 150, 202, 0.14), transparent 26%),
    radial-gradient(circle at top right, rgba(210, 161, 242, 0.12), transparent 24%),
