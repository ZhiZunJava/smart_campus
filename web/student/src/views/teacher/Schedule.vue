<template>
  <div class="portal-page schedule-page">
    <div class="schedule-header-user">
      <h3>{{ userStore.user?.realName || userStore.user?.userName || '老师' }}({{ userStore.user?.userId || '--' }})</h3>
    </div>
      ...
      <div class="table-container !pb-5">
        <div class="schedule-print-sheet">
          ...
          <div class="table-head-info">
            <span><label>院系：</label><span>{{ printTeacherInfo.college }}</span></span>
            <span><label>教师：</label><span>{{ printTeacherInfo.name }}</span></span>
            <span><label>编号：</label><span>{{ printTeacherInfo.teacherNo }}</span></span>
            <span><label>课程数：</label><span>{{ printTeacherInfo.courseCount }}</span></span>
            <span><label>班级数：</label><span>{{ printTeacherInfo.classCount }}</span></span>
            <span><label>周次：</label><span>{{ printTeacherInfo.weekLabel }}</span></span>
          </div>
          ...
          <div class="course-table-fontSize course-table-footer course-table-footer--print">
            <span>班级课程数：{{ classCourseCount }}</span>
...
const printSchoolTitle = computed(() => {
  ...
  return `${user.schoolName || user.orgName || user.deptName || '校园智学'}教师课表`
})
...
const printTeacherInfo = computed(() => {
  const user = userStore.user || {}
  return {
    college: user.deptName || user.dept?.deptName || '--',
    name: user.realName || user.nickName || user.userName || '老师',
    teacherNo: user.teacherNo || user.jobNumber || user.userId || '--',
    courseCount: classCourseCount.value,
    classCount: new Set(activities.value.map((item: any) => item.classId).filter(Boolean)).size,
    weekLabel: `第${currentWeek.value}周`,
  }
})
...
function buildPrintStyles() {
  return `
    ...
    .table-head-info > span:nth-child(1) { width: 16%; }
    .table-head-info > span:nth-child(2),
    .table-head-info > span:nth-child(3),
    .table-head-info > span:nth-child(4) { width: 17%; }
    .table-head-info > span:nth-child(5),
    .table-head-info > span:nth-child(6) { width: 16%; }
    .courseTable {
      width: 100%;
      margin-bottom: 0;
      font-size: 12px;
