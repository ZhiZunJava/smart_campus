<template>
  <div class="app-container">
    <el-form :inline="true" :model="queryParams" class="mb16">
      <el-form-item label="显示名称"><el-input v-model="queryParams.modelName" style="width: 220px"
    ...
    <el-table v-loading="loading" :data="dataList">
      ...
      <el-table-column label="默认" width="80">
        <template #default="scope"><el-tag :type="scope.row.isDefault === '1' ? 'success' : 'info'">{{
      ...
      <el-table-column label="状态" width="100"><template #default="scope"><el-tag
    ...
    <el-dialog v-model="open" :title="form.modelId ? '编辑模型' : '新增模型'" width="760px">
      <el-form :model="form" label-width="120px">
        <el-form-item label="显示名称"><el-input v-model="form.modelName"
            placeholder="如 DeepSeek / 阿里百炼 Qwen / 智谱 GLM" /></el-form-item>
        <el-form-item label="提供商">
          <el-select v-model="form.provider" @change="handleProviderChange">
            ...
            <el-option label="阿里百炼（Qwen）" value="qwen" />
        ...
        <el-form-item label="默认模型编码"><el-input v-model="form.modelCode"
            placeholder="如 deepseek-chat / qwen-plus / qwen-max" /></el-form-item>
        <el-form-item label="思考模型编码"><el-input v-model="form.reasoningModelCode"
            placeholder="如 deepseek-reasoner，可为空" /></el-form-item>
        <el-form-item label="视觉模型编码"><el-input v-model="form.visionModelCode"
            placeholder="支持图片时填写，可为空" /></el-form-item>
        ...
        <el-form-item label="是否默认"><el-switch v-model="defaultSwitch" inline-prompt active-text="是"
            inactive-text="否" /></el-form-item>
        <el-form-item label="优先级"><el-input-number v-model="form.priority" :min="0" :max="999"
            controls-position="right" /><span class="ml12 tip-text">越大越优先</span></el-form-item>
        <el-form-item label="适用业务"><el-input v-model="form.bizTypes"
            placeholder="如 qa,report,recommend,common" /></el-form-item>
        <el-form-item label="状态"><el-select v-model="form.status"><el-option label="正常" value="0" /><el-option
              label="停用" value="1" /></el-select></el-form-item>
      </el-form>
      <template #footer><el-button @click="open = false">取消</el-button><el-button type="primary"
          @click="submitForm">确定</el-button></template>
    </el-dialog>
    ...
    <el-dialog v-model="testOpen" title="模型测试结果" width="720px">
      <el-form label-width="90px">
        ...
        <el-form-item>
          ...
          <span class="ml12 tip-text">优先使用 Stream 实时返回，不支持时自动回落普通测试。</span>
        </el-form-item>
        <el-form-item label="思考过程">
          <el-input :model-value="testReasoning || '无'" type="textarea" rows="6" readonly />
        </el-form-item>
        <el-form-item label="最终回复">
          <el-input :model-value="testAnswer || '无'" type="textarea" rows="8" readonly />
        </el-form-item>
        <el-form-item label="Token">
          <el-input :model-value="testUsageText" readonly />
        </el-form-item>
      </el-form>
    </el-dialog>
  </div>
...
const currentTestSupportsStream = ref(false);
const testPrompt = ref("请简单介绍一下你当前模型连接是否正常。");
const testReasoning = ref("");
const testAnswer = ref("");
const testTokenUsed = ref<number | null>(null);
...
const supportStreamSwitch = computed({
  get: () => form.supportStream === "1",
  set: (value: boolean) => {
    form.supportStream = value ? "1" : "0";
  },
});
const supportReasoningSwitch = computed({
  get: () => form.supportReasoning === "1",
  set: (value: boolean) => {
    form.supportReasoning = value ? "1" : "0";
  },
});
const supportVisionSwitch = computed({
  get: () => form.supportVision === "1",
  set: (value: boolean) => {
    form.supportVision = value ? "1" : "0";
  },
});
...
function handleProviderChange(value: string) {
  ...
  if (value === "qwen") {
    ...
    form.modelName = form.modelName || "阿里百炼 Qwen";
    form.modelCode = form.modelCode || "qwen-plus";
    form.visionModelCode = form.visionModelCode || "qwen-vl-plus";
    form.supportStream = "1";
    form.supportVision = "1";
  }
  if (value === "zhipu") {
...
async function handleDelete(row: any) {
  ...
  await delAiModel(row.modelId);
  ElMessage.success("删除成功");
  getList();
}
function handleTest(row: any) {
  currentTestModelId.value = row.modelId;
  currentTestSupportsStream.value = row.supportStream === "1";
  testReasoning.value = "";
  testAnswer.value = "";
  testTokenUsed.value = null;
  testOpen.value = true;
}
async function submitTest() {
  if (!currentTestModelId.value) return;
  testLoading.value = true;
  testReasoning.value = "";
  testAnswer.value = "";
  testTokenUsed.value = null;
  try {
    if (currentTestSupportsStream.value) {
      await submitStreamTest();
      return;
    }
    const res = await testAiModel({
      modelId: currentTestModelId.value,
      prompt: testPrompt.value,
    });
    testReasoning.value = res.data?.reasoningContent || "";
    testAnswer.value = res.data?.content || "无返回内容";
    testTokenUsed.value =
      typeof res.data?.tokenUsed === "number" ? res.data.tokenUsed : null;
  } finally {
    testLoading.value = false;
  }
}

async function submitStreamTest() {
  const response = await fetch(testAiModelStreamUrl(), {
    method: "POST",
    headers: {
    ...
    for (const block of blocks) {
      ...
      if (eventName === "done") {
        try {
          ...
          testTokenUsed.value =
            typeof parsed?.aiResponse?.tokenUsed === "number"
              ? parsed.aiResponse.tokenUsed
              : testTokenUsed.value;
        } catch {
          // ignore parse error for done payload
        }
        return;
      }
      if (eventName === "error") {
        try {
          const parsed = JSON.parse(rawData);
          testAnswer.value = parsed?.message || rawData;
        } catch {
          testAnswer.value = rawData;
        }
        continue;
      }
      try {
        ...
        if (eventName === "reasoning") {
          testReasoning.value += parsed.content || "";
        } else {
          testAnswer.value += parsed.content || "";
        }
      } catch {
        testAnswer.value += rawData;
      }
