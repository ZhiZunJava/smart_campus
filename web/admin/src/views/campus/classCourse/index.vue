      ...
      <el-table-column label="班级课程" min-width="300">
        <template #default="{ row }">
          <div class="entity-cell">
            <strong>{{ row.courseName || getOptionLabel(courseOptions, row.courseId, '课程') }}</strong>
      ...
      <div class="dialog-dual-grid">
        ...
        <div class="dialog-panel">
          ...
          <el-form :model="form" label-width="92px">
            ...
            <el-form-item label="周学时"><el-input-number v-model="form.weeklyHours" :min="0" :max="20" style="width:100%" @change="syncTeachingLoad('weekly')" /></el-form-item>
...
import { addClassCourse, delClassCourse, generateTeachingClassCode, listClassCourse, updateClassCourse } from '@/api/campus/teaching'
...
function resetForm(){ Object.assign(form,{ id:undefined,classId:undefined,courseId:undefined,teacherId:undefined,termId:undefined,businessType:'专科',teachingClassCode:'',credits:0,courseCategory:'理论',openDeptId:undefined,major:undefined,campusName:'本部',assessmentType:'考查',teachingLanguage:'中文授课',prerequisiteCourse:'无',taskType:'正常',requiredFlag:'N',courseLevelRequirement:'无',totalHours:0,requiredWeeks:15,weeklyHours:0,arrangedHours:0,studentLimit:0,status:'0',remark:'' }) }
...
function handleUpdate(row?:any){ const item=row || dataList.value.find((i:any)=>i.id===ids.value[0]); if(!item) return; resetForm(); Object.assign(form,item); title.value='修改班级课程'; open.value=true }
