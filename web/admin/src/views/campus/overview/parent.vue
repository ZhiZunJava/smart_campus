<template>
  <div class="app-container">
    ...
    <el-row :gutter="16">
      <el-col :span="8"><el-card><div class="label">家长ID</div><div class="value">{{ data.parentUserId || '--' }}</div></el-card></el-col>
      <el-col :span="8"><el-card><div class="label">学生ID</div><div class="value">{{ data.studentUserId || '--' }}</div></el-card></el-col>
    ...
    <el-card class="mt16" shadow="never">
      ...
      <div class="info-text">家长概览已移除学习画像、预警与报告模块，仅保留基础关联信息与考试统计。</div>
    </el-card>
  </div>
</template>
...
async function loadOptions() {
  ...
  if (!queryParams.studentUserId && studentOptions.value.length) {
    queryParams.studentUserId = studentOptions.value[0].value
  }
}

async function loadData() {
  try {
    const res = await getParentDashboard(queryParams)
    data.value = res.data || {}
  } catch (_error) {
    ElMessage.error('暂无家长绑定学生信息')
  }
}
...
.value { font-size: 28px; font-weight: 700; color: var(--el-color-primary); }
